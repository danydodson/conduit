...
const mapDispatchToProps = dispatch => ({
  onLoad: (upload) =>
    dispatch({ type: UPLOADER_FORM_LOADED, upload }),
  onUpdate: (upload) =>
    dispatch({ type: UPLOADER_UPDATE_UPLOAD, upload }),
  // onAddUpload: (upload) =>
  //   dispatch({ type: EDITOR_ADD_UPLOAD, upload }),
  onUploaded: (uploads) =>
...
class Uploader extends Component {
  ...
  onMediaSelect = files => {
    ...
    for (let file of files) {
      ...
      request.post(process.env.REACT_APP_CLOUD_POST_URL)
        .field('upload_preset', process.env.REACT_APP_CLOUD_PRESET)
        .field('file', file)
        .field('multiple', true)
        ...
        .end((err, response) => this.onItemUploaded(uploadId, fileName, response, file))
    }
  }
  ...
  onItemUploaded(id, fileName, response, file) {
