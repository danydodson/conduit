...
const mapDispatchToProps = dispatch => ({
  onDeleteUploadedPhoto: () =>
    dispatch({ type: DELETE_UPLOADED_PHOTO })
})
...
class UploadStatus extends Component {
  render() {
    ...
    return (
      <div>
        ...
        {data && data.delete_token && (<button onClick={this.deletePhoto.bind(this)}>Delete image</button>)}
        <div className="status">
          {!response && <div>Uploading... {percent}%</div>}
          {!response && <div>In progress</div>}
          {response && (<div className="status-code">Upload completed with status code {response.status}</div>)}
        </div>
  ...
  deletePhoto() {
    request.post(`https://api.cloudinary.com/v1_1/scenicloud/delete_by_token`)
